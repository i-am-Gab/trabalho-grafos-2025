def salvar_solucao(path_saida, nome_instancia, rotas, custo_total, mapa_servicos, deposito, tempo_execucao=0, tempo_solucao=0):
    with open(f"{path_saida}/sol-{nome_instancia}.dat", "w") as f:
        ...
        for i, rota in enumerate(rotas, 1):
            ...
            f.write(f"0 1 {i} {rota['carga']} {int(rota['custo_total'])}  {total_visitas} (D 0,{deposito},{deposito}) ")
            f.write(" ".join(visitas))
            f.write(f" (D 0,{deposito},{deposito})\n")